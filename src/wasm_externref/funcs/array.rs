...
func_registry! {
    ...
    len => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>| -> AnyResult<u32> {
        Ok(site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?.len() as _)
    },
    get => |mut ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>, i: u32| -> AnyResult<Option<Rooted<ExternRef>>> {
        let v = site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?;
    ...
    set => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>, i: u32, x: Option<Rooted<ExternRef>>| -> AnyResult<()> {
        let mut v = site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?;
    ...
    reverse => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>| -> AnyResult<()> {
        site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?.reverse();
        Ok(())
    },
    sort => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>| -> AnyResult<()> {
        site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?.sort_unstable();
        Ok(())
    },
    ...
    clear => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>| -> AnyResult<()> {
        site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?.clear();
        Ok(())
    },
    remove => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>, i: u32| -> AnyResult<()> {
        site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?.remove(i as _);
        Ok(())
    },
