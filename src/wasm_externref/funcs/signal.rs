...
func_registry! {
    ...
    object => |mut ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>| -> AnyResult<Option<Rooted<ExternRef>>> {
        let v = site_context!(from_var_any::<Signal>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?;
    ...
    emit => |mut ctx: Caller<'_, T>, v: Option<Rooted<ExternRef>>, f: Option<Func>| -> AnyResult<()> {
        let c = site_context!(from_var_any::<Signal>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?;
