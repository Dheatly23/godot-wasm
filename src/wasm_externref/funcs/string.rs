...
func_registry! {
    ...
    len => |ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>| -> AnyResult<u32> {
        let v = site_context!(from_var_any::<GString>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?;
    ...
    read => |mut ctx: Caller<'_, _>, v: Option<Rooted<ExternRef>>, p: u32| -> AnyResult<u32> {
        ...
        match mem.data_mut(&mut ctx).get_mut(p as _..) {
            Some(mut s) => site_context!(write!(&mut s, "{}", v)).map_err(AnyError::from_anyhow)?,
            None => bail_with_site_wasm!("Invalid memory range ({}..)", p),
        };
        Ok(1)
    },
    write => |mut ctx: Caller<'_, _>, p: u32, n: u32| -> AnyResult<Option<Rooted<ExternRef>>> {
        ...
        let v = match mem.data(&mut ctx).get(p as _..(p + n) as _) {
            Some(s) => site_context!(from_utf8(s)).map_err(AnyError::from_anyhow)?.to_variant(),
            None => bail_with_site_wasm!("Invalid memory range ({}..{})", p, p + n),
        };
    ...
    from_string_name => |mut ctx: Caller<'_, T>, v: Option<Rooted<ExternRef>>| -> AnyResult<Option<Rooted<ExternRef>>> {
        let v = site_context!(from_var_any::<StringName>(&externref_to_variant(ctx.as_context(), v)?)).map_err(AnyError::from_anyhow)?;
