...
use wasmtime::{
    ...
    Result as AnyResult, Rooted, StoreContextMut, TypedFunc,
};
    ...
    ($(
        ...
        func_registry!{
            ...
            len => |ctx: Caller<'_, _>, a: Option<Rooted<ExternRef>>| -> AnyResult<u32> {
                let a = site_context!(from_var_any::<$t>(&externref_to_variant(ctx.as_context(), a)?)).map_err(AnyError::from_anyhow)?;
                Ok(a.len() as _)
            },
...
func_registry! {
    ...
    len => |ctx: Caller<'_, _>, a: Option<Rooted<ExternRef>>| -> AnyResult<u32> {
        let a = site_context!(from_var_any::<PackedByteArray>(&externref_to_variant(ctx.as_context(), a)?)).map_err(AnyError::from_anyhow)?;
        Ok(a.len() as _)
    },
    read => |mut ctx: Caller<'_, _>, a: Option<Rooted<ExternRef>>, p: u32| -> AnyResult<u32> {
        let a = site_context!(from_var_any::<PackedByteArray>(&externref_to_variant(ctx.as_context(), a)?)).map_err(AnyError::from_anyhow)?;
        ...
        site_context!(mem.write(&mut ctx, p as _, a.as_slice())).map_err(AnyError::from_anyhow)?;
        Ok(1)
    },
    write => |mut ctx: Caller<'_, _>, p: u32, n: u32| -> AnyResult<Option<Rooted<ExternRef>>> {
        let mem = match ctx.get_export("memory") {
...
func_registry! {
    ...
    get_many => |mut ctx: Caller<'_, _>, a: Option<Rooted<ExternRef>>, i: u32, f: Option<Func>| -> AnyResult<u32> {
        let f: TypedFunc<Option<Rooted<ExternRef>>, u32> = match f {
            Some(f) => site_context!(f.typed(&ctx)).map_err(AnyError::from_anyhow)?,
            None => return Ok(0),
        };
        let a = site_context!(from_var_any::<PackedStringArray>(
            &externref_to_variant(ctx.as_context(), a)?
