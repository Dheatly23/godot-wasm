...
func_registry! {
    ...
    from_instance_id => |mut ctx: Caller<'_, _>, id: i64| -> AnyResult<Option<Rooted<ExternRef>>> {
        let Some(id) = InstanceId::try_from_i64(id) else {
            bail_with_site_wasm!("Instance ID is 0")
        };

        variant_to_externref(ctx.as_context_mut(), site_context!(<Gd<Object>>::try_from_instance_id(id).map_err(|e| e.into_erased())).map_err(AnyError::from_anyhow)?.to_variant())
    },
    instance_id => |ctx: Caller<'_, _>, obj: Option<Rooted<ExternRef>>| -> AnyResult<i64> {
        site_context!(from_var_any::<Gd<Object>>(&externref_to_variant(ctx.as_context(), obj)?).map(|o| o.instance_id().to_i64())).map_err(AnyError::from_anyhow)
    },
    ...
    call => |mut ctx: Caller<'_, T>, obj: Option<Rooted<ExternRef>>, name: Option<Rooted<ExternRef>>, f: Option<Func>| -> AnyResult<Option<Rooted<ExternRef>>> {
        let mut obj = site_context!(from_var_any::<Gd<Object>>(&externref_to_variant(ctx.as_context(), obj)?)).map_err(AnyError::from_anyhow)?;
        let name = site_context!(from_var_any::<StringName>(&externref_to_variant(ctx.as_context(), name)?)).map_err(AnyError::from_anyhow)?;
        ...
        let r = ctx.data_mut().as_mut().release_store(move || site_context!(obj.try_call(&name, &v))).map_err(AnyError::from_anyhow)?;
    ...
    call_deferred => |mut ctx: Caller<'_, T>, obj: Option<Rooted<ExternRef>>, name: Option<Rooted<ExternRef>>, f: Option<Func>| -> AnyResult<Option<Rooted<ExternRef>>> {
        ...
        let r = ctx.data_mut().as_mut().release_store(move || site_context!(obj.try_call_deferred(&name, &v))).map_err(AnyError::from_anyhow)?;
        variant_to_externref(ctx.as_context_mut(), r)
    },
    callv => |mut ctx: Caller<'_, T>, obj: Option<Rooted<ExternRef>>, name: Option<Rooted<ExternRef>>, args: Option<Rooted<ExternRef>>| -> AnyResult<Option<Rooted<ExternRef>>> {
        let mut obj = site_context!(from_var_any::<Gd<Object>>(&externref_to_variant(ctx.as_context(), obj)?)).map_err(AnyError::from_anyhow)?;
        let name = site_context!(from_var_any::<StringName>(&externref_to_variant(ctx.as_context(), name)?)).map_err(AnyError::from_anyhow)?;
        let args = site_context!(from_var_any::<VarArray>(&externref_to_variant(ctx.as_context(), args)?)).map_err(AnyError::from_anyhow)?;
    ...
    get_indexed => |mut ctx: Caller<'_, T>, obj: Option<Rooted<ExternRef>>, path: Option<Rooted<ExternRef>>| -> AnyResult<Option<Rooted<ExternRef>>> {
        let obj = site_context!(from_var_any::<Gd<Object>>(&externref_to_variant(ctx.as_context(), obj)?)).map_err(AnyError::from_anyhow)?;
        let path = site_context!(from_var_any::<NodePath>(&externref_to_variant(ctx.as_context(), path)?)).map_err(AnyError::from_anyhow)?;
    ...
    connect => |ctx: Caller<'_, _>, obj: Option<Rooted<ExternRef>>, signal: Option<Rooted<ExternRef>>, target: Option<Rooted<ExternRef>>, flags: u32| -> AnyResult<()> {
        let mut obj = site_context!(from_var_any::<Gd<Object>>(&externref_to_variant(ctx.as_context(), obj)?)).map_err(AnyError::from_anyhow)?;
        let signal = site_context!(from_var_any::<StringName>(&externref_to_variant(ctx.as_context(), signal)?)).map_err(AnyError::from_anyhow)?;
        let target = site_context!(from_var_any::<Callable>(&externref_to_variant(ctx.as_context(), target)?)).map_err(AnyError::from_anyhow)?;

        match obj.connect_flags(&signal, &target, ConnectFlags::from_godot(flags as u64)) {
            GError::OK => Ok(()),
    ...
    emit_signal => |mut ctx: Caller<'_, T>, obj: Option<Rooted<ExternRef>>, name: Option<Rooted<ExternRef>>, f: Option<Func>| -> AnyResult<()> {
        ...
        site_context!(match ctx.data_mut().as_mut().release_store(move || site_context!(obj.try_emit_signal(&name, &v))).map_err(AnyError::from_anyhow)? {
            GError::OK => Ok(()),
