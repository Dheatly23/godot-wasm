    ...
    for crate::godot_component::GodotCtx
{
    ...
    fn from_list(
        ...
        let v: Array<Variant> = val.into_iter().map(|v| self.maybe_get_var(v)).collect();
    ...
    fn to_list(
        ...
        let v: Array<Variant> = self.get_var(var).try_to()?;
    ...
    fn resize(
        ...
        let mut v: Array<Variant> = self.get_var(var).try_to()?;
    ...
    fn extend(
        ...
        let mut v: Array<Variant> = self.get_var(var).try_to()?;
        v.extend_array(self.get_var(other).try_to()?);
        Ok(())
    }
