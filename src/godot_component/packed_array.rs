    ...
    ($m:ident <$t:ty>) => {
        ...
        impl<T: AsMut<GodotCtx>> $m::Host for T {
            ...
            fn to(&mut self, var: WasmResource<Variant>) -> AnyResult<Vec<$m::Elem>> {
                Ok(self.as_mut().get_var_borrow(var)?.try_to::<$t>()?.to_vec())
            }
            ...
            fn slice(
                ...
                let v: $t = self.as_mut().get_var_borrow(var)?.try_to()?;
            ...
            fn len(&mut self, var: WasmResource<Variant>) -> AnyResult<u32> {
                Ok(self.as_mut().get_var_borrow(var)?.try_to::<$t>()?.len() as _)
            }
            ...
            fn subarray(
                ...
                let v: $t = this.get_var_borrow(var)?.try_to()?;
