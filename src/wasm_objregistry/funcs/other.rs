...
func_registry! {
    "",
    delete => |mut ctx: Caller<T>, i: u32| -> Result<u32, Error> {
        match ctx.data_mut().as_mut().get_registry_mut()?.unregister(i as _) {
            Some(_) => Ok(1),
            None => Ok(0),
        }
    },
    delete_many => |mut ctx: Caller<T>, p: u32, n: u32| -> Result<u32, Error> {
    ...
    copy => |mut ctx: Caller<T>, s: u32, d: u32| -> Result<u32, Error> {
        let reg = ctx.data_mut().as_mut().get_registry_mut()?;
